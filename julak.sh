...
# ---------------------------------- #
if [ "${EUID}" -ne 0 ]; then
echo "${r}Anda perlu menjalankan skrip ini sebagai root ${NC}"
sleep 2
exit 0
fi
...
# Repository Github
jamban="https://raw.githubusercontent.com/hsspunya99/JVN/main/"
...
echo -e "${r} Arsitektur tidak Didukung ( ${y}$( uname -m )${NC} )"
exit 1
fi
...
echo -e " ${r}Script ini hanya support untuk OS"
echo -e ""
...
data_ip="https://raw.githubusercontent.com/hsspunya93/JVN/main/ip"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/rajaganjil93"
...
clear
echo -e "   ========================="
echo -e "   |\e[1;32mPilih Tipe Domain \e[0m|"
echo -e "   ========================="
echo -e "     \e[1;32m1)\e[0m Domain Sendiri"
echo -e "     \e[1;32m2)\e[0m Domain dari autoscript "
echo -e "   ========================="
read -p "   Silahkan masukkan nomor 1 atau 2 : " julak
echo ""
if [[ $julak == "1" ]]; then
echo -e "   \e[1;32mMasukkan domain anda $NC"
read -p "   Subdomain: " julak1
echo "IP=" >> /var/lib/julak/ipvps.conf
echo $julak1 > /etc/xray/domain
echo $julak1 > /root/domain
echo ""
elif [[ $julak == "2" ]]; then
# Random domain dari script
...
function pasang_domain() {
    ...
    echo "IP=" >> /var/lib/julak/ipvps.conf
    mkdir -p /etc/{xray,bot,vmess,vless,trojan,ssh,noobzvpns,limit,usr}
    ...
    touch /opt/julak/data/vmess.db
    touch /opt/julak/data/vless.db
    touch /opt/julak/data/trojan.db
    touch /opt/julak/data/ssh.db
    touch /opt/julak/data/bot.db
    echo "& plughin Account" >>/opt/julak/data/vmess.db
    echo "& plughin Account" >>/opt/julak/data/vless.db
    echo "& plughin Account" >>/opt/julak/data/trojan.db
...
# Simpan info port dll
wget -q -O /etc/info.txt "${jamban}info.txt"
...
# install badvpn
cd
wget -q -O /usr/sbin/badvpn "${jamban}service/badvpn" >/dev/null 2>&1
...
wget -q -O /etc/default/dropbear "${jamban}service/dropbear"
...
wget -q -O /etc/squid/squid.conf "${jamban}config/squid3.conf"
...
clear
cd
wget -q -O /usr/bin/limit-ip "${jamban}service/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
clear
wget -q -O /usr/bin/limit-ip-ssh "${jamban}service/limit-ip-ssh"
chmod +x /usr/bin/*
cd /usr/bin
...
[Unit]
Description=Python Proxy By JULAK BANTUR
...
insdns() {
wget -q ${jamban}service/installsl.sh && chmod +x installsl.sh && bash installsl.sh
clear
}
...
UDP="https://raw.githubusercontent.com/hsspunya93/JVN/main/config/"
# install udp-custom
...
USRID=$(curl https://raw.githubusercontent.com/hsspunya93/JVN/main/ip | grep $ipsaya | awk '{print $2}')
USREXP=$(curl https://raw.githubusercontent.com/hsspunya93/JVN/main/ip | grep $ipsaya | awk '{print $3}')
TIME=`date -d "0 days" +"%d-%m-%Y" `
TIMEZONE=$(printf '%(%H:%M:%S)T')
TEXT="
<code>────────────────────</code>
<b>⚠️AUTOSCRIPT JVN V1⚠️</b>
<code>────────────────────</code>
